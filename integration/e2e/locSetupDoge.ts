...
import { PublicKey } from "../../../psy-doge-solana-bridge/clients/js/node_modules/@solana/web3.js";

type Profile = "real-noop" | "initialized-noop" | "wormhole" | "legacy-ibc" | "testnet";
type Component = "dogecoin" | "electrs" | "solana" | "initialize" | "users" | "noop-monitor" | "legacy-ibc" | "block-sender" | "ibc-pipeline" | "manager-service" | "deposit" | "withdraw";
...
const IBC_REPO = path.resolve(SCRIPT_DIR, "../..");
const DEFAULT_PROJECTS_DIR = path.resolve(IBC_REPO, "..");
...
const TESTNET_ELECTRS_URL = "https://doge-electrs-testnet-demo.qed.me";
const BLOCK_SENDER_PORT = 3000;
const MANAGER_SERVICE_PORT = 7071;
const BLOCK_SENDER_API_TOKEN = "local-e2e-token";
...
const FULL_E2E_FUNDING_SCHEMA = "psy-doge-full-e2e-funding-v1";
...
Local components and lifecycle:
  --dogecoind                 Force local Dogecoin regtest plus local electrs (default for real/initialized profiles)
  --dogecoin / --no-dogecoin  Enable/disable the local Dogecoin regtest component
...
One-shot operator modes:
  --deposit                   Run deposit_to_solana in the foreground
  --funding-wif <WIF>         Funding Dogecoin private key in WIF form
  ...
  --recipient-token-account <pubkey>
                              Recipient Solana token account
  --withdraw                  Run process_withdrawal in the foreground
...
function parseProfile(value: unknown): Profile {
    ...
    if (!["real-noop", "initialized-noop", "wormhole", "legacy-ibc", "testnet"].includes(profile)) {
        fail(`Unknown profile '${profile}'. Expected real-noop, initialized-noop, wormhole, legacy-ibc, or testnet.`);
    }
...
function resolveOptions(): Options | null {
    ...
    if (values.full && profile !== "testnet") fail("--full is only supported with --profile testnet.");
    const projectsDir = path.resolve(asString(values["projects-dir"], DEFAULT_PROJECTS_DIR));
    const fullE2eFundingRaw = values["prepare-full-e2e-funding"];
    if (fullE2eFundingRaw !== undefined && String(fullE2eFundingRaw).trim().length === 0) {
    ...
    if (values.deploy) {
        ...
        deploy = {
            ...
            manifestPath: path.resolve(asString(values.manifest, path.join(SCRIPT_DIR, "deployments", `${cluster}-doge-bridge.json`))),
    ...
    switch (profile) {
        case "real-noop":
            components = new Set(["dogecoin", "solana"]);
            break;
        case "initialized-noop":
            components = new Set(["dogecoin", "solana", "initialize", "users"]);
            break;
        case "wormhole":
            components = new Set();
            break;
        case "legacy-ibc":
            components = new Set(["legacy-ibc"]);
            break;
        case "testnet":
            components = new Set(["solana", "initialize", "users"]);
            if (values.full) {
                components.add("block-sender");
                components.add("ibc-pipeline");
                components.add("manager-service");
            }
            break;
    }
    ...
    if (values.only) {
        ...
        const allowed: Record<Component, true> = {
            ...
            "noop-monitor": true, "legacy-ibc": true, "block-sender": true,
            "ibc-pipeline": true, "manager-service": true, deposit: true, withdraw: true,
        };
    ...
    if (fullE2eFundingArtifact) {
        if (profile !== "real-noop" && profile !== "initialized-noop") fail("--prepare-full-e2e-funding is only supported by local real-noop/initialized-noop profiles.");
        if (values["no-dogecoin"]) fail("--prepare-full-e2e-funding cannot be combined with --no-dogecoin.");
    ...
    if (deposit) {
        const missing = [
            ["--funding-wif", values["funding-wif"]],
    ...
    let dogecoind = Boolean(values.dogecoind || profile === "real-noop" || profile === "initialized-noop");
    if (values["no-dogecoin"] && !values.dogecoind) dogecoind = false;
    if (values.dogecoin || values.electrs || values.only?.split(",").some((component) => ["dogecoin", "electrs"].includes(component.trim()))) dogecoind = true;
    ...
    return {
        ...
        deposit,
        withdraw,
        fundingWif: values["funding-wif"] ? String(values["funding-wif"]) : undefined,
...
class Launcher {
    ...
    constructor(options: Options) {
        ...
        if (!options.deploy && options.profile !== "wormhole") {
            this.programs.push({
    ...
    keypairPubkey(keypair: string): string {
        return this.capture(["solana-keygen", "pubkey", keypair], IBC_REPO);
    }
    ...
    programShow(id: string): Record<string, string> {
        const deploy = this.options.deploy!;
        const output = this.capture(["solana", "program", "show", id, "--url", deploy.rpcUrl, "--output", "json"], IBC_REPO);
    ...
    async preflightDeployment(): Promise<{ genesisHash: string; solanaVersion: string; payerPubkey: string; balanceLamports: number; actions: Record<string, "deploy" | "upgrade"> }> {
        ...
        const bridgeCli = path.join(this.options.bridgeRepo, "target/release/doge-bridge-cli");
        ...
        if (cliBuildRequired) console.log("  (build initializer) cargo build --release -p doge-bridge-cli");
    ...
    async deployPublic(): Promise<void> {
        ...
        if (deploy.initialize && !firstExecutable([path.join(this.options.bridgeRepo, "target/release/doge-bridge-cli")])) {
            const command = ["cargo", "build", "--release", "-p", "doge-bridge-cli"];
        ...
        if (deploy.airdropSol !== undefined) {
            ...
            await this.run(command, "public-airdrop", IBC_REPO);
    ...
    async preflightWormhole(): Promise<void> {
        ...
        if (executableInPath("docker")) {
            const docker = this.syncOutput(["docker", "info"], IBC_REPO);
        ...
        if (executableInPath("kubectl")) {
            const context = this.syncOutput(["kubectl", "config", "current-context"], IBC_REPO);
            ...
            const cluster = this.syncOutput(["kubectl", "cluster-info"], IBC_REPO);
            if (!cluster.ok) errors.push(`Kubernetes cluster is unreachable: ${cluster.stderr || cluster.stdout}`);
            const namespace = this.syncOutput(["kubectl", "get", "namespace", WORMHOLE_NAMESPACE, "--ignore-not-found", "-o", "name"], IBC_REPO);
    ...
    async preflight(): Promise<{ dogeRunning: boolean; solanaRunning: boolean; electrsRunning: boolean }> {
        ...
        if (this.options.fullE2eFundingArtifact) {
            try { assertWritableNewPath(this.options.fullE2eFundingArtifact); } catch (error) { fail(`Full-E2E funding artifact path is not safely writable: ${String(error)}`); }
        }
        ...
        if (c.has("manager-service") && !firstExecutable([this.localOpsBinary("local_manager_service")])) {
            if (this.options.noBuild) fail(`Manager service is missing and --no-build was set: ${this.localOpsBinary("local_manager_service")}`);
            if (!executableInPath("cargo")) fail("Building local manager service requires cargo.");
        }
    ...
    async buildMissing(status: { dogeRunning: boolean; solanaRunning: boolean; electrsRunning: boolean }): Promise<{ dogeDaemon: string | null; dogeCli: string | null; electrs: string | null }> {
        ...
        if (c.has("manager-service") && !firstExecutable([this.localOpsBinary("local_manager_service")])) {
            await this.run(["cargo", "build", "--release", "--bin", "local_manager_service"], "build-manager-service", this.localOpsRoot());
            if (!firstExecutable([this.localOpsBinary("local_manager_service")])) fail(`Manager service build did not produce ${this.localOpsBinary("local_manager_service")}`);
        }
    ...
    async startWormhole(): Promise<void> {
        ...
        await waitFor("Solana devnet pod readiness", 20 * 60_000, async () => {
            const r = this.syncOutput(["kubectl", "-n", WORMHOLE_NAMESPACE, "wait", "--for=condition=Ready", "pod", "-l", "app=solana-devnet", "--timeout=30s"], IBC_REPO);
        ...
        await waitFor("Guardian pod readiness", 20 * 60_000, async () => {
            const r = this.syncOutput(["kubectl", "-n", WORMHOLE_NAMESPACE, "wait", "--for=condition=Ready", "pod", "-l", "app=guardian", "--timeout=30s"], IBC_REPO);
            return r.ok ? r.stdout : null;
        });
        ...
        await waitFor("Spy pod readiness", 20 * 60_000, async () => {
            const r = this.syncOutput(["kubectl", "-n", WORMHOLE_NAMESPACE, "wait", "--for=condition=Ready", "pod", "-l", "app=spy", "--timeout=30s"], IBC_REPO);
            return r.ok ? r.stdout : null;
        });
    ...
    async prepareFullE2eFunding(): Promise<void> {
        const artifactPath = this.options.fullE2eFundingArtifact;
        ...
        console.log(`[funding] Mining exactly ${FULL_E2E_FUNDING_BLOCKS} blocks to fresh address ${address}.`);
        const mined = await jsonRpc(DOGE_RPC_URL, "generatetoaddress", [FULL_E2E_FUNDING_BLOCKS, address], rpcAuth);
        if (!Array.isArray(mined) || mined.length !== FULL_E2E_FUNDING_BLOCKS) {
            fail(`generatetoaddress returned ${Array.isArray(mined) ? mined.length : "non-array"} block hashes; expected ${FULL_E2E_FUNDING_BLOCKS}.`);
        }
        const indexedHeight = await waitFor("Electrs indexing full-E2E funding blocks", 180_000, async () => {
            const indexed = await electrsHeight();
            if (indexed === null) return null;
        ...
        const candidates = (await fundingUtxos(address))
            .filter((utxo) => {
                if (!utxo.status.confirmed || utxo.status.block_height === undefined || utxo.value < FULL_E2E_MIN_VALUE_SATS) return false;
                return indexedHeight - utxo.status.block_height + 1 >= FULL_E2E_MIN_CONFIRMATIONS;
            })
            .sort((left, right) =>
        ...
        if (!selected || selected.status.block_height === undefined) {
            fail(`No funding UTXO has at least ${FULL_E2E_MIN_CONFIRMATIONS} confirmations and ${FULL_E2E_MIN_VALUE_SATS} satoshis.`);
        }
        ...
        writeJsonAtomic(artifactPath, {
            schema: FULL_E2E_FUNDING_SCHEMA,
            createdAt: new Date().toISOString(),
            network: "regtest",
            address,
            wif: wifValue,
            ...
            confirmations,
            minedBlocks: FULL_E2E_FUNDING_BLOCKS,
            minimumConfirmations: FULL_E2E_MIN_CONFIRMATIONS,
            minimumValue: FULL_E2E_MIN_VALUE_SATS,
    ...
    localOpsRoot(): string {
        return path.join(this.options.projectsDir, "psy-doge-solana-cli", "doge");
    }

    localOpsBinary(name: string): string {
        return path.join(this.localOpsRoot(), "target", "release", name);
    }

    electrsUrl(): string {
        return this.options.dogecoind ? ELECTRS_HTTP_URL : TESTNET_ELECTRS_URL;
    }
    ...
    async prepareBlockPipelineConfig(): Promise<string[]> {
        ...
        return [
            ...
            "--deposit-evidence-path", "/tmp/e2e-test-deposit-evidence.json",
    ...
    async startIbcCheckpointRedis(): Promise<void> {
        const name = `psy-doge-e2e-checkpoint-${this.runId}`.toLowerCase().replace(/[^a-z0-9_.-]/g, "-");
        const result = Bun.spawnSync([
            "docker", "run", "-d", "--rm",
            "--name", name,
            "--label", `psy.doge.launcher.run=${this.runId}`,
        ...
        if (result.exitCode !== 0) fail(`Failed to start E2E checkpoint Redis container: ${result.stderr.toString()}`);
        const id = result.stdout.toString().trim();
        this.state.containers.push({ role: "ibc-checkpoint-redis", id, name });
        this.persist();
        await waitFor("IBC checkpoint Redis TCP", 30_000, async () => await portOpen(PORTS.legacyRedis));
        console.log(`[ready:ibc-checkpoint] Redis ${id.slice(0, 12)} on 127.0.0.1:${PORTS.legacyRedis}`);
    }
    ...
    async startBlockSender(): Promise<void> {
        ...
        if (!fs.existsSync(path.join(senderDir, "dist", "index.js"))) {
            console.warn("[block-sender] dist/index.js not found, run pnpm build in solana-doge-bridge-block-sender first");
            return;
        }
        const signerKeypair = this.bridgeKeys().operator;
        if (!fs.existsSync(signerKeypair)) fail(`Block sender signer keypair is missing: ${signerKeypair}. Initialize the bridge first.`);
        const signerBytes = JSON.parse(fs.readFileSync(signerKeypair, "utf8")) as unknown;
        if (!Array.isArray(signerBytes) || signerBytes.length < 32 || signerBytes.slice(0, 32).some((value) => !Number.isInteger(value) || value < 0 || value > 255)) {
            fail(`Block sender signer keypair is invalid: ${signerKeypair}`);
        }
        const signerSecretHex = Buffer.from(signerBytes.slice(0, 32) as number[]).toString("hex");
        await this.spawn(
            ["node", "dist/index.js"],
            "block-sender",
            senderDir,
            true,
            {
                ...
                SIGNER_KEYPAIR: signerKeypair,
                SOL_PAYER_SECRET_KEY: signerSecretHex,
    ...
    async startIbcPipeline(): Promise<void> {
        ...
        await this.spawn([
            ...
            "--redis-url", `redis://127.0.0.1:${PORTS.legacyRedis}`,
    ...
    async startManagerService(): Promise<void> {
        const bin = this.localOpsBinary("local_manager_service");
        await this.spawn([bin, "--listen", `127.0.0.1:${MANAGER_SERVICE_PORT}`], "manager-service", undefined, true);
    ...
    async runDeposit(options: { fundingWif: string; fundingTxid: string; fundingVout: number; fundingAmount: number; recipientTokenAccount: string }): Promise<void> {
        ...
        ], "deposit-to-solana", this.localOpsRoot());
    }
    ...
    async start(): Promise<void> {
        ...
        if (c.has("ibc-pipeline") && !c.has("legacy-ibc")) await this.startIbcCheckpointRedis();
