...
use qed_dsol_ibc_node_common::{doge_link_rpc_async::DogeLinkElectrsAsyncClient, nimpl::{proof_store_fred::ProofStoreFred, simple_submitter::SimpleBlockSubmitter}, sol_submitter::SolSubmitterClient, worker::{simple_async_block_processor::SimpleAsyncBlockProcessor, simple_async_fake_prover::SimpleAsyncFakeProver}};



async fn run_fake_g16_worker() -> anyhow::Result<()> {

    let mut timer = DebugTimer::new("fake_g16_worker_ex1");
    timer.lap("start");
    ...
    let mut prover = SimpleAsyncFakeProver::new();
...
#[tokio::main]
async fn main() {
    run_fake_g16_worker().await.unwrap();
}